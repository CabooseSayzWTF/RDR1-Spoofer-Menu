...
bool menu_pressed_right2 = false;
bool menu_pressed_left2 = false;
bool welcomemsg = true;
//extern bool enableonce = true;
...
//this base has a bug where if you use scrollable char items in more than one submenu, it breaks all other scroll submenus eventually
//so this is a copy to temp fix that because we only need two submenus with scrollable char items
void AddScrollableItem2(char *str, char *scrollable[], void *func, void *backfunc, const char *msg, int *index)
{
	if (AddBaseItem(str, func, backfunc, msg, false))
	{
		stradd_s(menu_text, ": ");
	...
	if (AddBaseItem(str, func, backfunc, msg, false))
	{
		...
		if (selected_index == current_submenu_count - 1)
		{
			if (menu_pressed_right2)
			{
				...
				WAIT(50);
				menu_pressed_right2 = false;
			}

			if (menu_pressed_left2)
			{
				if (*index > min)
				{
					(*index)--;
